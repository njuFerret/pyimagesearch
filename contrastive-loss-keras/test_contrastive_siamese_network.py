...
print("[INFO] Loading test image dataset...")
...
for (i, (pathA, pathB)) in enumerate(pairs):
    ...
    # Create a copy of both the images for visualization purposes
    origA = imageA.copy()
    origB = imageB.copy()
    # Add a channel dimension to both the images
    imageA = np.expand_dims(imageA, axis=-1)
    imageB = np.expand_dims(imageB, axis=-1)
    # Add a batch dimension to both the images
    imageA = np.expand_dims(imageA, axis=0)
    imageB = np.expand_dims(imageB, axis=0)
    # Scale the pixel values to the range [0, 1]
    imageA = imageA / 255.0
    imageB = imageB / 255.0
    # Use the siamese model to make predictions on the image pair, indicating whether or not
