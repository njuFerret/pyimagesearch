...
def detect_faces(net, image, minConfidence=0.5):
    ...
    for i in range(0, detections.shape[2]):
        ...
        # Filter out any weak detections by ensuring the confidence is greater than the minimum confidence needed
    ...
    # Return the face detection bounding box results
    return boxes


def load_face_dataset(inputPath, net, minConfidence=0.5, minSamples=15):
    # Grab the paths to all images in the input directory, extract the name of the person (i.e, class label) from
    # the directory structure and count the number of example images per face
    ...
    for imagePath in imagePaths:
        ...
        for (startX, startY, endX, endY) in boxes:
            # Extract the face ROI, resize it and the convert the image to grayscale
